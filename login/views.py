...
def generate_token(user_id, username, role):
    access_token_expiry = timedelta(seconds=settings.TIME["ACCESS_TOKEN_MAX_AGE"])
    refresh_token_expiry = timedelta(seconds=settings.TIME["REFRESH_TOKEN_MAX_AGE"])
...
def refresh_token_view(request):
    ...
    try:
        ...
        data = jwt.decode(
            refresh_token, settings.KEYS["JWT_SECRET_KEY"], algorithms=["HS256"]
        )
        ...
        access_token, new_refresh_token = generate_token(user_id, username, role)

        # Set new tokens in cookies
        response = redirect("/home")  # Redirect to the protected home view or dashboard
        response.set_cookie(
...
def login_required(fn):
    def wrapped_view(request, *args, **kwargs):
        ...
        if not access_token:
            return redirect("/")  # Redirect to login if access token is missing

        try:
            # Decode the access token
            data = jwt.decode(
                access_token, settings.KEYS["JWT_SECRET_KEY"], algorithms=["HS256"]
            )
