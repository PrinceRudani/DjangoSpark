...
def admin_load_product(request):
    try:
        ...
        return render(
            request,
            "admin/product_templates/addProduct.html",
            {
        ...
        return AppServices.handle_exception("Failed to load product form", 500, e)
...
def admin_load_ajax(request):
    try:
        ...
        logger.info(f"Fetching subcategories for category ID: {product_category_id}")
    ...
    try:
        if request.method == "POST" and request.FILES.get("product_image"):
            ...
            product_vo.product_image_path = os.path.join(
                "static", "product_image", product_image.name
            )
            product_vo.save()
            logger.info(f"Product '{product_vo.product_name}' inserted successfully")
...
def admin_view_product(request):
    try:
        ...
        return render(
            request,
            "admin/product_templates/viewProduct.html",
...
def admin_edit_product(request, product_id):
    try:
        ...
        sub_category_vo_lst = SubCategoryVO.objects.filter(
            subcategory_category_vo=product_vo.product_category_id, is_deleted=False
        )
        return render(
            request,
            "admin/product_templates/updateProduct.html",
            {
...
def admin_update_product(request, product_id):
    try:
        if request.method == "POST":
            ...
            product_vo.save()
            logger.info(f"Updated product '{product_vo.product_name}' successfully")
            return redirect("admin_view_product")
...
def product_json(request):
    cate = ProductVO.objects.all()
    serializer = ProductSerializer(cate, many=True)
