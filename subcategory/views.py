...
def admin_edit_subcategory(request):
    try:
        ...
        return render(
            ...
            {"subcategory_vo": subcategory_vo, "category_vo_lst": category_vo_lst},
        )
    except Exception as e:
...
def admin_update_subcategory(request, subcategory_id):
    try:
        ...
        subcategory_vo.subcategory_description = request.POST["subCategoryDescription"]
        subcategory_vo.save()
...
@login_required
def subcategory_json(request):
    cate = SubCategoryVO.objects.all()
    serializer = SubcategorySerializer(cate, many=True)
