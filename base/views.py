...
def load_home_page(request):
    if request.method == 'POST':
        try:
            ...
            # Validate user role
            if user.login_role not in [settings.ROLES['ADMIN'],
                                       settings.ROLES['USER']]:
                raise ValueError("Invalid user role.")

            # Generate new tokens
            access_token, refresh_token = generate_token(user.login_id,
                                                         user.login_username,
                                                         user.login_role)

            # Set tokens in cookies
            response = render(request, 'admin/home.html')

            response.set_cookie(
                settings.TOKENS['ACCESSTOKEN'], access_token,
                max_age=settings.TIME['ACCESS_TOKEN_MAX_AGE'],
                httponly=True
            )
            response.set_cookie(
                settings.TOKENS['REFRESHTOKEN'], refresh_token,
